...
const app = {
    state: {
        ...
        agendaFilter: 'today', // 'today', 'pending', 'completed'
        appointments: [],
        services: []
    },
    ...
    renderSettings(container) {
        container.innerHTML = `
            ...
            <div style="text-align: center; margin-top: 40px; opacity: 0.5;">
                <p style="font-size: 0.8rem; font-weight: 600;">Agenda Studio v1.5 (Supabase)</p>
...
const app = {
    state: {
        ...
        appointments: JSON.parse(localStorage.getItem('appointments')) || [
            { id: 1, client: 'María López', service: 'Esmaltado Semi', date: '2024-02-14', time: '10:00', price: 15000, worker: 'Manuela', status: 'pending' },
            { id: 2, client: 'Ana García', service: 'Esculpidas', date: '2024-02-14', time: '12:00', price: 25000, worker: 'Manuela', status: 'completed' }
        ],
        services: JSON.parse(localStorage.getItem('coquette_services')) || [
            { name: 'Esmaltado Semi', price: 15000 },
            { name: 'Esculpidas', price: 25000 },
            { name: 'Kapping', price: 18000 },
            { name: 'Nail Art Pro', price: 8000 }
        ]
    },
            ...
            <div class="card">
                ...
                ${this.state.services.map(s => `
                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 12px 0; border-bottom: 1px dashed var(--primary);">
                        ...
                        ${this.state.currentRole === 'admin' ? `
                            <div style="display: flex; gap: 8px;">
                                <button class="btn btn-outline" style="padding: 6px 10px; font-size: 0.65rem; width: auto;" onclick="app.showEditService('${s.name}')">✏️</button>
                                <button class="btn btn-outline" style="padding: 6px 10px; font-size: 0.65rem; width: auto; border-color: #ff8ba0; color: #ff5c8a;" onclick="app.deleteService('${s.name}')">🗑️</button>
                            </div>
                        ` : ''}
                    </div>
                `).join('')}
    ...
    renderSettings(container) {
        container.innerHTML = `
            ...
            <div style="text-align: center; margin-top: 40px; opacity: 0.5;">
                <p style="font-size: 0.8rem; font-weight: 600;">Agenda Studio v1.3</p>
                <p style="font-size: 0.7rem;">Hecho con ❤️ para tu negocio</p>
            </div>
        `;
    },
    ...
    showAddService() {
        ...
        document.getElementById('service-form').onsubmit = (e) => {
            ...
            const name = document.getElementById('service-name').value;
            const price = parseInt(document.getElementById('service-price').value);
            this.state.services.push({ name, price });
            this.saveServices();
            this.closeModal();
            this.render();
        };
    ...
    showEditService(name) {
        ...
        modalContent.innerHTML = `
            <div style="text-align: center; margin-bottom: 20px;">
                <h2 style="color: var(--secondary-vibrant); font-size: 1.6rem;">Editar Servicio ✨</h2>
                <p style="opacity:0.6; font-size: 0.9rem;">Modifica el precio de ${name}</p>
            </div>
        ...
        document.getElementById('edit-service-form').onsubmit = (e) => {
            e.preventDefault();
            service.price = parseInt(document.getElementById('edit-service-price').value);
            this.saveServices();
            this.closeModal();
            this.render();
        };
    ...
    deleteService(name) {
        if (confirm(`¿Estás segura de eliminar el servicio "${name}"?`)) {
    ...
    saveServices() {
        localStorage.setItem('coquette_services', JSON.stringify(this.state.services));
    },
...
window.onload = () => app.init();
>>>>>>> 7452f18023f95f9e14a74994f3ae58f455804f63
